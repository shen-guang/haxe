...
let get_macro_context ctx p =
	...
	| None ->
		...
		com2.class_path <- List.filter (fun vd -> not (ExtString.String.exists vd#full_path "/_std/")) com2.class_path;
		let name = platform_name !Globals.macro_platform in
		com2.class_path <- List.map (fun p -> new Vfs.sys_directory (name ^ "/_std/") (Some p)) com2.std_path @ com2.class_path;
